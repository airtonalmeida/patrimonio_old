...
public class BemEditLoteMB extends AbstractEditPageBean<Bem, Long> {
	...
	@Transactional
	public List<Bem> listaBemLote(Integer quantidade){
		List<Bem> bemLoteList = new ArrayList<Bem>();
		
		Bem bemLote = null;
		
		for (int i = 0; i<quantidade; i++) {
			
			bemLoteList.add(bemLote);
	...
	@Transactional
	public String inserirLote() {
				
		Integer quantidade = getQuantidadeLote();
		
		List<Bem> listaBens = listaBemLote(quantidade);
		
		Iterator<Bem> it = listaBens.iterator();
		...
		try {
			while(it.hasNext()){	
			    ...
			    bem.setTombo(getBean().getTombo());
			    bem.setTomboInteger(getBean().getTomboInteger());
			    bem.setDescricao(getBean().getDescricao());
			    bem.setTipoBem(getBean().getTipoBem());
			    bem.setSubTipoBem(getBean().getSubTipoBem());
				bem.setBase(getBean().getBase());
				bem.setMinisterio(getBean().getMinisterio());
				bem.setMarca(getBean().getMarca());
				bem.setModelo(getBean().getModelo());
				bem.setTipoAquisicao(getBean().getTipoAquisicao());
				bem.setResponsavel(getBean().getResponsavel());
				bem.setLocalizacao(getBean().getLocalizacao());
				bem.setNumeroNotaFiscal(getBean().getNumeroNotaFiscal());
				bem.setSituacao(getBean().getSituacao());
				bem.setNota(getBean().getNota());
				
				bem.setDataCompra(getBean().getDataCompra());
				
				bem.setValorHistoricoString(getBean().getValorHistoricoString());
				bem.setValorHistorico(getBean().getValorHistorico());
				bem.setValorAtualString(getBean().getValorAtualString());
								
											
				this.bemBC.insert(bem);
						
						
			}
			
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "CONFIRMAÇÃO", quantidade + " bens cadastrados com sucesso!");
