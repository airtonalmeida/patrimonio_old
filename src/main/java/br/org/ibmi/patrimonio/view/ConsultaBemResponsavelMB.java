...
@ViewController
//@NextView("./consultaPorResponsavelEditar.jsf")
@NextView("./bem_edit.jsf")
@PreviousView( "./consultaPorResponsavel.jsf")
public class ConsultaBemResponsavelMB extends AbstractEditPageBean<Bem, Long> {
	...
	@Transactional
	public void consultarBemPorResponsavel() {
		
		if(getBean().getResponsavel()==null){
			...
			FacesMessage messageNull = new FacesMessage(FacesMessage.SEVERITY_INFO, "AVISO", "Selecione o Responsável!");
			
			RequestContext.getCurrentInstance().showMessageInDialog(messageNull);
			
		}else{
		
			resultado = bemBC.consultarListBemResponsavelBC(getBean());
			...
			if(!resultado.isEmpty()){
				...
				FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "AVISO", "Não há Bem cadastrado com o Responsável " + getBean().getResponsavel().getNome());
		...
		if(getBean().getResponsavel()==null){
			...
			RequestContext.getCurrentInstance().showMessageInDialog(messageNull);
				...
				if(!resultado.isEmpty()){
					...
					relatorio.showReportResponsavel(resultado);
