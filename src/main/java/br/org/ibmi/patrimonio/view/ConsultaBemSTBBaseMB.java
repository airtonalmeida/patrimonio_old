...
@ViewController
//@NextView( "./consultaPorSTBBaseEditar.jsf")
@NextView("./bem_edit.jsf")
@PreviousView("./consultaPorSubTipoBemBase.jsf")
public class ConsultaBemSTBBaseMB extends AbstractEditPageBean<Bem, Long> {
	...
	@Transactional
	public void conBemSubTpBemBase() {
		
		if((getBean().getSubTipoBem()==null)||(getBean().getBase()==null)){
			...
			FacesMessage messageNull = new FacesMessage(FacesMessage.SEVERITY_INFO, "AVISO", "Selecione o Sub Tipo do Bem e a Base!");
			
			RequestContext.getCurrentInstance().showMessageInDialog(messageNull);
			
		}else{
		
				resultado = bemBC.conListBemSubTpBemBaseBC(getBean());
				...
				if(!resultado.isEmpty()){
					...
					FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "AVISO", "Não há Bem cadastrado com o Sub Tipo de Bem " + getBean().getSubTipoBem().getDescricao() + " e a Base " + getBean().getBase().getDescricao());
		...
		if((getBean().getSubTipoBem()==null)||(getBean().getBase()==null)){
			...
			RequestContext.getCurrentInstance().showMessageInDialog(messageNull);
				...
				if(!resultado.isEmpty()){
					...
					relatorio.showReportSTBBase(resultado);
