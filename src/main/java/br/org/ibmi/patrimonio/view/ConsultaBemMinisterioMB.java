...
@ViewController
//@NextView("./consultaPorMinisterioEditar.jsf")
@NextView("./bem_edit.jsf")
@PreviousView("./consultaPorMinisterio.jsf")
public class ConsultaBemMinisterioMB extends AbstractEditPageBean<Bem, Long> {
	...
	@Transactional
	public void consultarBemPorMinisterio() {
		
		if(getBean().getMinisterio()==null){
			...
			FacesMessage messageNull = new FacesMessage(FacesMessage.SEVERITY_INFO, "AVISO", "Selecione o Ministério!");
			
			RequestContext.getCurrentInstance().showMessageInDialog(messageNull);
			
		}else{
		
			resultado = bemBC.consultarListBemMinisterioBC(getBean());
			...
			if(!resultado.isEmpty()){
				...
				FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "AVISO", "Não há Bem cadastrado com o Ministério " + getBean().getMinisterio().getDescricao());
		...
		if(getBean().getMinisterio()==null){
			...
			RequestContext.getCurrentInstance().showMessageInDialog(messageNull);
				...
				if(!resultado.isEmpty()){
					...
					relatorio.showReportMinisterio(resultado);
